...
public class RoomGenerator : MonoBehaviour
{
    ...
    public GameObject smokePrefab; // Prefab dymu

    public GameObject enemyPrefab;
    public GameObject chestPrefab;
    public GameObject filarPrefab;

    public Weapon[] ancientWeapons;
    public Weapon[] middleAgeWeapons;
    public Weapon[] modernWeapons;
    ...
    public (bool[,], List<(int, int)>) GenerateBools(int width, int height)
    {
        ...
        if (seed <= 0) seed += (int)(((System.DateTime.Now.Millisecond * 100) + System.DateTime.Now.Second) * Time.deltaTime);
        if (seed > 10000) { seed %= 100; }
        UnityEngine.Random.InitState(seed);
        //first coordinate is spawn point
        var spawnpoint = (UnityEngine.Random.Range(2, width - 2), UnityEngine.Random.Range(2, height - 2));
        bools[spawnpoint.Item1, spawnpoint.Item2] = true;

        var exitpoint = (UnityEngine.Random.Range(2, width - 2), UnityEngine.Random.Range(2, height - 2));
        while (exitpoint.Item1 == spawnpoint.Item1 || exitpoint.Item2 == spawnpoint.Item2)
        {
            exitpoint = (UnityEngine.Random.Range(2, width - 2), UnityEngine.Random.Range(2, height - 2));
        }
    ...
    public IEnumerator GenerateRoomCo()
    {
        ...
        Tile tile;
        int idx = 0;
        int numOfTiles = UnityEngine.Random.Range(5, Enumerable.Range(0, Tiles.GetLength(0)).Count(i => Tiles[i, idx] != null) / 3);
        ...
        for (int i = 0; i < numOfTiles; i++)
        {
            ...
            do
            {
                tile = Tiles[UnityEngine.Random.Range(0, Tiles.GetLength(0)), UnityEngine.Random.Range(0, Tiles.GetLength(1))];
                if (tile != null && tile.Wall == null)
                {
                    if (tile.ObjectsSpawnedOnTile.Count == 0)
                        done = true;
                }


            } while (!done);
            tile.ObjectsSpawnedOnTile.Add(Instantiate(enemyPrefab, tile.transform.position, Quaternion.identity, tile.transform));
            ...
            tile.ObjectsSpawnedOnTile.Add(Instantiate(chestPrefab, tile.transform.position, Quaternion.identity, tile.transform));

            
            switch(AbsoluteManager.instance.PlayerCircle)
            {
                case <=1:
                    tile.ObjectsSpawnedOnTile.First().gameObject.GetComponentInChildren<Chest>().items = ancientWeapons;
                    break;
                case 2:
                    tile.ObjectsSpawnedOnTile.First().gameObject.GetComponentInChildren<Chest>().items = middleAgeWeapons;
                    break;
                case >2:
                    tile.ObjectsSpawnedOnTile.First().gameObject.GetComponentInChildren<Chest>().items = modernWeapons;
                    break;
            }
            ...
            tile.ObjectsSpawnedOnTile.Add(Instantiate(filarPrefab, tile.transform.position, Quaternion.identity, tile.transform));
        }
            ...
            for (int y = 0; y < bools.GetLength(1); y++)
            {
                ...
                if (thisTile)
                {
                    ...
                    if (numberOfConnections > 1)
                    {
                        ...
                        {
                            var spawner = Instantiate(PlayerSpawnPrefab, position, Quaternion.identity, tile.transform).GetComponent<OnTileObj>();

                            tileScript.onTileObjects.Add(spawner);
                            tileScript.onTileObjects[0].x = x;
                            tileScript.onTileObjects[0].y = y;
                            tileScript.onTileObjects[0].size = 1;
                            tileScript.isFree = false;
                            PlayerController.instance.transform.position = spawner.GetComponentInChildren<PlayerSpawner>().transform.position;

                            continue;
                        }
                        if (tileScript.x == exitpoint.Item1 && tileScript.y == exitpoint.Item2)
                        {
                            tileScript.onTileObjects.Add(Instantiate(StairwayPrefab, new Vector3(position.x, position.y - 4, position.z - 2), Quaternion.identity, tile.transform).GetComponent<OnTileObj>());
                            tileScript.onTileObjects[0].x = x;
                            tileScript.onTileObjects[0].y = y;
                            tileScript.onTileObjects[0].size = 1;
                            tileScript.isFree = false;
                            tileScript.GetComponent<Collider>().enabled = false;
                            foreach (var ren in tile.GetComponentsInChildren<MeshRenderer>())
                            {
                                if (ren.gameObject.name.Contains("plane")) { ren.enabled = false; }
                                break;
                            }
                ...
                {
                    Vector3 position = new Vector3(x * spacing, 0, y * spacing);
                    Instantiate(smokePrefab, position, Quaternion.identity, Parent);
                }
            }
        }
